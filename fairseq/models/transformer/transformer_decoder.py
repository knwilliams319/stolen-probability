...
class TransformerDecoderBase(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        if cfg.use_alibi_attention:
            ...
            self.alibi = self.alibi.repeat(cfg.max_tokens//maxpos, 1, 1)  # batch_size, 1, 1
    ...
    def buffered_future_mask_alibi(self, tensor):
        ...
        if (
            self._future_mask.size(0) == 0
            or (not self._future_mask.device == tensor.device)
            or self._future_mask.size(1) < self.cfg.tokens_per_sample
        ):
            self._future_mask = torch.triu(
                utils.fill_with_neg_inf(torch.zeros([self.cfg.tokens_per_sample, self.cfg.tokens_per_sample])), 1
            )
